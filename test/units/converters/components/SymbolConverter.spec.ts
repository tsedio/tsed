...
describe("SymbolConverter", () => {
  ...
  it("should do something", () => expect(!!this.symbolConverter).to.be.true);
  ...
  describe("serialize()", () => {
    before(() => (this.symbolTest = this.symbolConverter.serialize(Symbol("testSymbol2"))));

    after(() => delete this.symbolTest);

    it("should serialize data to a string", () => expect(this.symbolTest).to.be.a("string"));
    it("should serialize data to a string that equals to testSymbol2", () => expect(this.symbolTest).to.be.equal("testSymbol2"));
  });
});

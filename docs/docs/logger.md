...
Ts.ED has his own logger available through [`@tsed/logger`](https://logger.tsed.io) package.

## Features

Ts.ED logger support many features, and it optimized to be used in production. 

- Many [layout](https://logger.tsed.io/layouts) format are supported:
...
- Many appender (stream) format are supported:
...
- @@RequestLogger@@ in **production** mode, cache all request logs until the response isn't send to your consumer. See (/docs/logger.md#request-logger).
...
::: tip
All log use throw ctx.logger will be associated with the uniq request id generated by Ts.ED.
:::

::: tip
@@RequestLogger@@ in **production** mode, cache all request logs until the response isn't send to your consumer.
:::

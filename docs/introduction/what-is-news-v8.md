...
You can also now subscribe to three new hooks: `$beforeInvoke`, `$afterInvok`, and `$beforeInvoke:${type}`.
...
V8 add a new package `@tsed/platform-http` that contains all the features related to the HTTP platform but without extra Ts.ED dependencies.

...
describe("SwaggerService", () => {
  ...
  describe("getOpenAPISpec()", () => {
    it("should compile spec only once time", async () => {
      ...
      vi.spyOn((swaggerService as any).injectorService, "alterAsync");
      ...
      expect((swaggerService as any).injectorService.alterAsync).toHaveBeenCalledTimes(1);
      expect((swaggerService as any).injectorService.alterAsync).toHaveBeenCalledWith("$alterOpenSpec", result1, {specVersion: "3.0.1"});
    });
  });
});

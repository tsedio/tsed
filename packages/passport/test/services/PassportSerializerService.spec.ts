...
import {PassportSerializerService, UserInfo} from "../../src";
...
describe("PassportSerializerService", () => {
  ...
  it("should deserialize model", inject([PassportSerializerService], async (service: PassportSerializerService) => {
    const result = await new Promise((resolve) => service.deserialize("{\"id\":\"id\",\"email\":\"email@email.fr\"}", (...args: any[]) => resolve(args)));

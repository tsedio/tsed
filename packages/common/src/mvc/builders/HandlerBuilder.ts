...
import {HandlerMetadata, IHandlerOptions} from "../class/HandlerMetadata";
...
export class HandlerBuilder {
   ...
   * @param locals
   */
  private async getHandler(locals: Map<string | Function, any>): Promise<Function> {
  ...
  private async invoke(request: Express.Request, response: Express.Response, next: any, err?: any): Promise<any> {
    ...
    try {
      ...
      const handler = await this.getHandler(container);
      const process = handler(...args);

...
The functional JsonSchema API lacks ergonomic helpers for composing object schemas. Users need to clone schemas manually to pick/omit properties or to merge multiple schema fragments, which is error-prone and leaves TypeScript inference (`s.infer()`) unaware of the resulting shapes. This gap also means there is no documentation or tests describing these workflows.

## Goals

1. Provide fluent `.pick<K>()` and `.omit<K>()` helpers on `JsonSchema` that create new schemas scoped to selected keys.
2. Provide a `.merge()` helper that combines multiple schema fragments, including other `JsonSchema` instances, while retaining metadata such as required fields.
3. Ensure TypeScript inference through `s.infer()` reflects the transformed shapes for all three helpers.
4. Cover the new behavior with unit tests (runtime & typing) and update the Functional API documentation in `docs/docs/model.md`.
...
- Extend `JsonSchema<T>` with `pick`, `omit`, and `merge` methods that operate on the internal `properties` map and `#required` set, returning cloned schemas to keep immutability expectations.
- Introduce shared utility typing (e.g., `SchemaPick<T, K>`, `SchemaOmit<T, K>`, `SchemaMerge<T, U>`) in `domain/types.ts` so TypeScript consumers benefit automatically via `s.infer()`.
...
- Functional API users can compose schemas through `pick`, `omit`, and `merge` without manually cloning maps.
- `s.infer()` yields the expected `Pick`, `Omit`, or intersection types when the helpers are used.
- Tests capture both runtime shape filtering/merging and compile-time inference.
